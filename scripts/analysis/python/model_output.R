...
# Replace IDs in the Category column with corresponding labels
for (i in seq_along(label$GeneWeaver_ID)) {
  pattern <- as.character(label$GeneWeaver_ID[i])
  replacement <- label$Label[i]
  model_1_matches$Category <- gsub(pattern, replacement, model_1_matches$Category)
}
...
for (file in names(dataframes_list)) {
  cat("Updated data frame for file:", file, "\n")
  #print(dataframes_list[[file]])
  cat("\n")
  cat("\n")
  # Get the first row and first column value
  first_value <- dataframes_list[[file]][1, 1]  # Extract the value from the first row and first column
  first_value_full <- Full_dataframes_list[[file]][1, 1]  # Extract the value from the first row and first column
  
  # Remove the "_0" pattern from the value
  cleaned_value <- gsub("_0", "", as.character(first_value))  
  print(cleaned_value)
  
    # Check if cleaned_value matches any GeneWeaver_ID in label data
  if (cleaned_value %in% as.character(label$GeneWeaver_ID)) {
    cleaned_label <- label$Label[as.character(label$GeneWeaver_ID) == cleaned_value]
    print(paste("Matched Label:", cleaned_label))
  } else {
    cleaned_label <- NA
    print("No match found in label data.")
  }
  ...
  pattern <- as.character(label$GeneWeaver_ID[i])
  replacement <- label$Label[i]
  model_match$Category <- gsub(pattern, replacement, model_match$Category)
	}
	
	print(model_match)

  
  csv_out <- paste(annot,"/HTML_model_N_output/","M_",cleaned_label,".csv",sep = "")
  fwrite(model_match,csv_out,sep=",")
  csv_html <- paste("/apps/conda/project/envs/imlabtools/bin/csvtotable ",csv_out," ",annot,"/HTML_model_N_output/M_",cleaned_label,".html",sep= "")
  print(csv_html)
  system(csv_html)
  
  cell_out <- paste(annot,"/HTML_model_N_output/","baseline_Include_",first_value_full,".csv",sep = "")
  fwrite(as.data.frame(Full_dataframes_list[[file]]),cell_out,sep=",")
